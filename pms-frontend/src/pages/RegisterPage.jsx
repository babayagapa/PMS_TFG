      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div className="glass" style={{ padding: '48px', maxWidth: '420px', width: '100%' }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={inputStyle}><label className="input-label">Nombre</label><input type="text" className="input-field" placeholder="Mario" value={form.nombre} onChange={e => set('nombre', e.target.value)} />{errores.nombre && <p style={{ color: '#e74c3c', fontSize: '12px', margin: '4px 0 0' }}>{errores.nombre}</p>}</div>
            <div style={inputStyle}><label className="input-label">Apellidos</label><input type="text" className="input-field" placeholder="Gómez Sánchez" value={form.apellidos} onChange={e => set('apellidos', e.target.value)} />{errores.apellidos && <p style={{ color: '#e74c3c', fontSize: '12px', margin: '4px 0 0' }}>{errores.apellidos}</p>}</div>
